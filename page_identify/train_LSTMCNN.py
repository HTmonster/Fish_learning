...
#模型超参数
tf.flags.DEFINE_integer("embedding_size",100,"隐藏层的维度")
tf.flags.DEFINE_integer("max_seq_length",100,"输入序列的最大长度")
...
print("\nLoading data...")
x_text, y = data_Processer.load_positive_negative_url_files(FLAGS.positive_url_file,FLAGS.negative_url_file)
print(x_text)
...
x_padding, max_x_length = data_Processer.padding_url(x_text,padding_url_length=FLAGS.max_seq_length)

# 映射编码 每个字符对应一个数字
print("padding done!")
# x= data_Processer.map_code_char(x_padding,max_x_length)
x,vocab_size=data_Processer.one_hot_char(x_padding,max_x_length)
